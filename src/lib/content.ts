...
export const projectFigure: Record<string, string> = {
  mantis: 'Runtime data path of MANTIS. On the client, the shared stem extracts early features from the input frame; the task detector estimates task relevance, P(task), from those features, and the modulator conditions the encoder through ten cGDN sites. The entropy-coded latent ẑ is transmitted over the uplink and routed at the edge to the corresponding task decoder and head. In the example shown, the frame contains a new smoke plume and is routed to smoke detection. Schematic: task probabilities, latent values, and outputs are illustrative.',
